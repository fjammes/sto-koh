...
public class AlgorithmGrowingCellStructures extends Algorithm {
    ...
    Vector<Vertex> fNeigh;
    Vector<Vertex> qNeigh;  
    ...
    public AlgorithmGrowingCellStructures(Vertex[] _data_,
            ...
            EPSILONB = _epsilonb_;
            EPSILONN = _epsilonn_;
            this.setAdaptationStep(iteration);
            init();
    }
    ...
    protected void iterate() {
        ...
        // Growth Phase
        if ((this.getAdaptationStep().getCounter() % LAMBDA == 0) && (graph.numVertices() <= nCellsMax)) {
            if (log.isDebugEnabled()) {
                System.out.print("\n\nGrowth Phase...\n");
            ...
            for (Iterator i = graph.getNeighbours(fFurthest); i.hasNext(); ) {
                ...
                if (!fNeigh.contains(candidate)) {
                    if (candidate != fFurthest) {
                        ...
                        if (log.isDebugEnabled()) System.out.print("fNeigh found " + candidate + "\n");
                    }
                }
            }
            ...
            for (Iterator i = graph.getNeighbours(hqc); i.hasNext(); ) {
                ...
                if (!qNeigh.contains(candidate)) {
                    if (candidate != hqc) {
                        ...
                        if (log.isDebugEnabled()) System.out.print("qNeigh found " + candidate + "\n");
                    }
                }
            }
            ...
            for (Iterator i = fNeigh.iterator(); i.hasNext(); ) {
                GCSVertex candidate = (GCSVertex)i.next();
                if (qNeigh.contains(candidate)) {
                    if (log.isDebugEnabled()) System.out.print("shared found " + candidate + "\n");
            ...
            if (log.isDebugEnabled()) System.out.print("created " + r.toString() + "\n");
        ...
        // Prunning Phase
        if (this.getAdaptationStep().getCounter() % LAMBDA_PRUNE == 0) {
            if (log.isDebugEnabled())
                System.out.print("\n\nPruning Phase...\n");
            GCSVertex candidate = null;
            for (Iterator i = graph.getAllVertices(); i.hasNext(); ) {
                GCSVertex di = (GCSVertex)i.next();
                if (log.isDebugEnabled()) System.out.print("  Processing: " + di.toString() + "\n");
                double distMax = 0.0d;
                double dist = 0.0d;
                int count = 0;
    ...
    public Counter getAdaptationStep() {
		return iteration;
	}

	public void setAdaptationStep(Counter iteration) {
